 ...
 *
 * Rules the page keeps: every number shows its source, licence and fetch date; every card shows
 * who is counted and who is not; meters that disagree are shown side by side, never averaged;
 * a fitted trend always carries its 90% band and is only drawn past the data when the reader
 * turns on "if the trend continued", labelled as not a forecast.
 */
(function () {
  'use strict';
  ...
  function trendLayers(t, xmax) {
    ...
    if (state.ext && t.if_trend_continued && t.if_trend_continued.length) {
      var e = [t.band[t.band.length - 1]].concat(t.if_trend_continued);
      L.push({ type: 'band', data: e.map(function (b) { return [b[0], b[1], b[3]]; }), logged: true, cls: 'trend-ext-band' });
      L.push({ type: 'line', data: e.map(function (b) { return [b[0], b[2]]; }), logged: true, cls: 'trend-ext',
  ...
  // enough up to hold a trend's 'if it continued' stretch when that is switched on
  function logRange(values, trends) {
    ...
    (trends || []).forEach(function (t) {
      if (state.ext && t && t.status === 'ok' && t.if_trend_continued) t.if_trend_continued.forEach(function (b) { hi = Math.max(hi, Math.ceil(b[3])); });
    });
    return [Math.pow(10, lo), Math.pow(10, hi)];
  }
  function trendXMax(t, xmax) {
    if (state.ext && t && t.status === 'ok' && t.if_trend_continued && t.if_trend_continued.length)
      return Math.max(xmax, t.if_trend_continued[t.if_trend_continued.length - 1][0]);
    return xmax;
  }
  function fitSentence(t, what) {
    if (!t) return '';
    if (t.status !== 'ok') return (what || t.label) + ': too few points to fit a trend (' + t.n + ').';
    var w = t.whole, ci = w.doubling_ci;
    var s = (what || t.label) + ', ' + fmtDate(w.from) + ' to ' + fmtDate(w.to) + ' (' + w.n + ' points): doubles every ' +
      months(w.doubling_months) + ' (90% range ' + (ci[0] != null ? months(ci[0]) : '?') + ' to ' +
      (ci[1] != null ? months(ci[1]) : 'no growth') + '), about ' + w.x_per_year + '× a year.';
    var ch = t.change || {};
    if (ch.verdict === 'too few points') s += ' Last 24 months: too few points to tell whether the pace changed.';
    else if (t.recent) s += ' Last 24 months alone: ' + months(t.recent.doubling_months) + ' (' + ch.verdict + ').';
    return s;
  }
  ...
  function coverage(parent, d, extra) {
    ...
    var p1 = h('p', null, null, box); h('b', null, 'Who is counted. ', p1); p1.appendChild(document.createTextNode(d.coverage.summary + ' ' + (d.coverage.counted || '')));
    var p2 = h('p', null, null, box); h('b', null, 'Who is not. ', p2); p2.appendChild(document.createTextNode(d.coverage.not_counted || ''));
    if (extra) { var p3 = h('p', null, null, box); p3.appendChild(document.createTextNode(extra)); }
    return box;
  }
  function sourceLine(parent, d, id) {
    var p = h('p', 'src', null, parent);
    p.appendChild(document.createTextNode('Source: '));
    link(d.page || d.url, d.source, p, true);
    p.appendChild(document.createTextNode(' · licence: '));
    link(d.licence_url || d.page, d.licence, p, true);
    p.appendChild(document.createTextNode(' · fetched ' + fmtDate(d.fetched_at, true) + ' · '));
  ...
  function table(parent, columns, rows, caption) {
    ...
    det.addEventListener('toggle', function once() {        // build rows on first open only
  ...
  var METER_ANCHOR = {
    ...
    arena_leaderboard: 'ovc', compute_frontier: 'm-compute', compute_notable: 'm-compute',
    compute_frontier_no_spec: 'm-compute', cost_frontier: 'm-cost', hw_price_performance: 'm-hardware',
  ...
  var METER_NAME = {
    ...
    arena_leaderboard: 'Arena votes'
  };
  function renderDisagree() {
    var ul = $('#disagree'); ul.textContent = '';
    (S.sentences || []).forEach(function (s) {
      var li = h('li', null, s.text, ul), m = h('span', 'meters', 'Meters: ', li);
      var seen = {};
      s.meters.forEach(function (id, i) {
        var name = METER_NAME[id] || (trend(id) || {}).label || id, a = METER_ANCHOR[id];
        if (seen[name]) return; seen[name] = 1;
        if (Object.keys(seen).length > 1) m.appendChild(document.createTextNode(' · '));
        if (a) link('#' + a, name, m); else m.appendChild(document.createTextNode(name));
      });
    });
    var asof = $('#asof');
    asof.textContent = 'Numbers as fetched up to ' + fmtDate(S.as_of, true) + '. They refresh weekly through a pull request that a person reads before it is merged.';
  }
  ...
  function renderSpeed() {
    ...
    (S.trends || []).forEach(function (t) {
      if (t.status !== 'ok') { rows.push({ label: t.label, marks: [], empty: 'too few points to fit', t: t }); return; }
      ...
      rows.push({ label: t.label, marks: marks, t: t, sort: t.whole.doubling_months || 999 });
    });
    (S.metr_quoted || []).forEach(function (q) {
      rows.push({ label: q.label + ' (METR\'s own fit)', marks: [{ v: q.doubling_months, lo: q.doubling_ci[0], hi: q.doubling_ci[1], kind: 'quoted', q: q }],
        sort: q.doubling_months, quoted: true });
    });
    rows.sort(function (a, b) { return (a.sort || 999) - (b.sort || 999); });
    mount(host, 'intervalChart', function () {
      ...
      return {
        rows: rows, aria: 'Months to double, by meter, with 90% ranges',
        x: { min: 2, max: 150, ticks: [3, 6, 12, 24, 48, 96], fmt: monthsTick },
        tooltip: function (d) {
          var m = d.mark, lines = [];
          if (m.kind === 'quoted') lines.push('Quoted from METR (' + (m.q.note || 'their fit') + ').');
          else {
            var src = m.kind === 'whole' ? m.t.whole : m.t.recent;
            lines.push((m.kind === 'whole' ? 'Whole period' : 'Last 24 months') + ': ' + fmtDate(src.from) + ' to ' + fmtDate(src.to) + ', ' + src.n + ' points');
            lines.push('About ' + src.x_per_year + '× a year');
            if (m.kind === 'whole') lines.push('Pace lately: ' + m.t.change.verdict);
          }
          return { value: months(m.v) + ' to double', lines: [d.row.label, '90% range: ' + (m.lo != null ? months(m.lo) : '?') + ' to ' + (m.hi != null ? months(m.hi) : 'no growth')].concat(lines) };
        },
        onPick: function (d) { var a = d.row.t ? METER_ANCHOR[d.row.t.id] : 'm-metr'; if (a) location.hash = a; }
      };
    });
    legend(host.parentNode, [{ label: 'whole period', color: css('--neutral') }, { label: 'last 24 months', color: css('--neutral'), shape: 'hollow' },
      { label: 'quoted from METR', color: css('--ink-3') }]);
  }
  ...
  function renderOpenClosed() {
    ...
    h('p', 'tv', n.lag_months != null ? 'About ' + Math.round(n.lag_months) + ' months behind' : 'Open is ahead', t1);
    h('p', 'td', n.lag_months != null ? ('The best open-weights model, ' + n.open_model + ' (' + n.open_org + ', ' + fmtDate(n.open_date) +
      '), scores ' + n.open_eci + '. A closed model first reached that in ' + fmtDate(n.closed_date) + ' (' + n.closed_model + ').' +
      (n.lag_range_months && n.lag_range_months[0] != null && n.lag_range_months[1] != null ? ' Range given the score\'s uncertainty: ' +
        Math.round(n.lag_range_months[0]) + ' to ' + Math.round(n.lag_range_months[1]) + ' months.' : '') +
      ' Gap today: ' + e.gap_points + ' index points.') : n.status, t1);
    // preference
    ...
    if (a && a.configs && (a.configs.text_style_control || a.configs.text)) {
      var cfg = a.configs.text_style_control || a.configs.text, lt = cfg.latest;
      h('p', 'tv', 'Best closed wins ' + pct(lt.win_prob) + ' of votes', t2);
      h('p', 'td', 'Best closed, ' + lt.closed_model + ' (' + lt.closed_org + '), against best open, ' + lt.open_model + ' (' + lt.open_org + ', ' +
        lt.open_licence + '): ' + Math.round(lt.gap) + ' rating points apart' + (lt.win_prob_range ? ', so between ' + pct(lt.win_prob_range[0]) + ' and ' + pct(lt.win_prob_range[1]) : '') +
        ' in a head-to-head vote (' + cfg.label + ', ' + fmtDate(lt.date, true) + '). 50% would be a coin flip.', t2);
    } else {
    ...
    // releases
    var r = ovc.release_share, full = r.series.filter(function (s) { return s[0] < r.partial_year && s[4] != null; }), last = full[full.length - 1],
      cur = r.series.filter(function (s) { return s[0] === r.partial_year; })[0], t3 = h('div', 'tile', null, tiles);
    h('p', 'tq', 'How many (notable releases)', t3);
    h('p', 'tv', pct(last[4]) + ' open in ' + last[0], t3);
    h('p', 'td', last[1] + ' of ' + (last[1] + last[2]) + ' notable models released in ' + last[0] + ' had downloadable weights' +
      (cur ? '; ' + pct(cur[4]) + ' so far in ' + cur[0] + ' (to ' + fmtDate(r.newest) + ')' : '') + '. This counts releases Epoch calls notable, not use or ability.', t3);
    // use: a meter we do not have
    var t4 = h('div', 'tile gap', null, tiles);
    h('p', 'tq', 'How much used (tokens served)', t4);
    h('p', 'tv', 'Not measured here', t4);
    h('p', 'td', 'We have not found an official, openly licensed download that counts how much open and closed models are actually used across the industry. Any single provider or router sees only its own traffic, so a share of use is one more vantage point, not the whole.', t4);
    ...
    mount($('#ovc-lag'), 'xyChart', function () {
      ...
      return {
        ...
        y: { min: Math.min(0, Math.floor(Math.min.apply(null, hist.map(function (x) { return x.lag_range_months[0] != null ? x.lag_range_months[0] : x.lag_months; })) / 3) * 3),
          max: Math.max(12, Math.ceil(Math.max.apply(null, hist.map(function (x) { return x.lag_range_months[1] || x.lag_months; })) / 3) * 3), fmt: function (v) { return v + ' mo'; }, title: 'months behind (below 0: open ahead)' },
        layers: [
        ...
        tooltip: function (d) {
          var x = d.row;
          return { value: x.lag_months + ' months behind', lines: [x.open_model + ' (' + x.open_org + ', ' + fmtDate(x.open_date, true) + '), index ' + x.open_eci,
            'Closed first reached it: ' + x.closed_model + ', ' + fmtDate(x.closed_date, true), 'Range: ' + x.lag_range_months[0] + ' to ' + x.lag_range_months[1] + ' months'] };
        },
    ...
    if (a && a.configs && Object.keys(a.configs).length) {
      mount($('#ovc-arena'), 'xyChart', function () {
        ...
        return {
          height: 210, aria: 'Chance the best closed model beats the best open one in an Arena vote, over time', crosshair: true,
          ...
          y: { min: Math.min(40, Math.floor(Math.min.apply(null, all.map(function (d) { return d.y; })) / 10) * 10),
               max: Math.max(80, Math.ceil(Math.max.apply(null, all.map(function (d) { return d.y; })) / 10) * 10), fmt: function (v) { return v + '%'; }, title: 'best closed wins' },
          layers: layers,
          tooltip: function (d) {
            var row = objs(D.arena_leaderboard).filter(function (r) { return r.config === d.cfg && r.date === d.p[0]; })[0] || {};
            return { value: 'Best closed wins ' + d.y.toFixed(0) + '%', lines: [a.configs[d.cfg].label + ', ' + fmtDate(d.p[0], true), 'Gap: ' + d.p[1] + ' points' + (d.p[2] != null ? ' (± about ' + d.p[2] + ')' : ''),
      ...
      legend($('#ovc-arena').parentNode, [{ label: 'style-adjusted votes (the arena\'s default view)', color: css('--ink'), shape: 'line' },
        { label: 'raw votes', color: css('--neutral-soft'), shape: 'line' }, { label: 'rough ± range', shape: 'band' }]);
    } else {
      $('#ovc-arena').textContent = 'Arena history is not available in this refresh.';
    }
    mount($('#ovc-share'), 'columnsChart', function () {
      var cats = r.series.map(function (s) {
        return { key: s[0], tick: (+s[0] % 2 === 0 || r.series.length < 8) ? "'" + s[0].slice(2) : '', incomplete: s[0] === r.partial_year, s: s,
      ...
      return {
        ...
        tooltip: function (c) {
          ...
          return { value: s[0] + (c.incomplete ? ' (so far)' : ''), rows: [
            { value: String(s[1]), label: 'open weights (' + pct(s[4]) + ')', color: css('--open') },
  ...
  function renderCompute(parent) {
    var d = D.epoch_training_compute, c = card(parent, 'm-compute', 'Training compute',
      'How much computation went into training each notable model? The fitted line follows the largest models of their day.');
    ...
    mount(host, 'xyChart', scatterSpec({
      aria: 'Training compute of notable models since 2010, log scale, with the frontier trend',
    ...
    kindLegend(c, [{ label: 'hollow: "Speculative" estimate', color: css('--ink-3'), shape: 'hollow' }, { label: 'fitted trend, 90% band', shape: 'band' }]);
    var t = trend('compute_frontier'), t2 = trend('compute_frontier_no_spec'), t3 = trend('compute_notable');
    h('p', 'fitline', fitSentence(t, 'Frontier models'), c);
    var more = h('p', 'fitline muted', null, c);
    more.textContent = (t2 && t2.status === 'ok' ? 'Leaving out "Speculative" estimates: ' + months(t2.whole.doubling_months) + '. ' : '') +
      (t3 && t3.status === 'ok' ? 'All notable models since 2018: ' + months(t3.whole.doubling_months) + ' over the whole period, but ' +
        (t3.recent ? months(t3.recent.doubling_months) + ' in the last 24 months, where most estimates come from labs that disclose (often smaller, open-weights models), so the flattening may be about who is counted.' : 'too few recent points.') : '');
    coverage(c, d);
  ...
  function renderShipped(parent) {
    ...
    mount(host, 'columnsChart', function () {
      ...
      return {
        height: 260, categories: cats, aria: 'AI compute shipped per quarter by chip designer',
        ...
        tooltip: function (cat) {
          return { value: 'Q' + cat.qn + ' ' + cat.q.slice(0, 4),
            rows: cat.parts.filter(function (p) { return p.value > 0; }).reverse().map(function (p) { return { value: compact(p.value), label: p.designer + (p.incomplete ? ' (still being counted)' : ''), color: p.color }; })
              .concat(cat.parts.some(function (p) { return !p.row; }) ? [{ value: '', label: 'no series yet: ' + cat.parts.filter(function (p) { return !p.row; }).map(function (p) { return p.designer; }).join(', ') }] : []) };
        }
      };
    });
  ...
  function renderHardware(parent) {
    var d = D.epoch_ml_hardware, c = card(parent, 'm-hardware', 'Chip speed per dollar',
      'How much 16-bit AI compute does one dollar of list price buy? Only chips with a public price can be placed.');
    var pos = S.positions.epoch_ml_hardware;
    nowLine(c, compact(pos.value, 0) + ' FLOP/s per $', 'best: ' + pos.who + ' (' + fmtDate(pos.date) + ')');
    var rows = objs(d).filter(function (r) { return r.flops_per_usd; }), host = h('div', null, null, c), pin = pinBox(c);
    var pts = rows.map(function (r) { return { x: decYear(r.date), y: r.flops_per_usd, r: 5, row: r, player: playerOf(r.maker) }; });
    var xs = pts.map(function (p) { return p.x; });
    mount(host, 'xyChart', scatterSpec({
      aria: 'FP16 FLOP per second per dollar of list price, by chip release date, log scale',
      points: function () { pts.forEach(function (p) { p.color = css('--neutral'); }); return pts; },
      trend: 'hw_price_performance', ylog: true, yfmt: function (v) { return compact(v, 0); }, ytitle: 'FLOP/s per dollar (log scale)',
      xmin: Math.floor(Math.min.apply(null, xs)), xmax: Math.max.apply(null, xs) + 0.3,
      tooltip: function (p) {
        var r = p.row;
        return { value: compact(r.flops_per_usd, 1) + ' FLOP/s per $', lines: [r.name + ' (' + (r.maker || '') + ', ' + fmtDate(r.date) + ')',
          compact(r.fp16_flops, 0) + ' FLOP/s FP16/BF16 at a list price of ' + usd(r.price_usd)] };
      },
      onPick: function (p) {
        var r = p.row;
        showPin(pin, p, [r.name + ': ' + compact(r.flops_per_usd, 1) + ' FLOP/s per dollar', (r.maker || '') + ' · ' + fmtDate(r.date) + ' · list price ' + usd(r.price_usd)],
          r.link ? { href: r.link, text: 'Datasheet' } : { href: d.page, text: 'Epoch AI hardware data' });
      }
    }));
    legend(c, [{ label: 'a chip with a list price', color: css('--neutral') }, { label: 'fitted trend, 90% band', shape: 'band' }]);
    h('p', 'fitline', fitSentence(trend('hw_price_performance'), 'Priced chips'), c);
    coverage(c, d, 'List prices are not what hyperscalers pay, and consumer cards sit next to data-centre chips here, so read the level loosely and the trend with care.');
    sourceLine(c, d, 'epoch_ml_hardware');
    table(c, [{ label: 'Chip', get: function (r) { return r.name; }, href: function (r) { return r.link; } }, { label: 'Maker', get: function (r) { return r.maker; } },
      { label: 'Released', get: function (r) { return r.date; } }, { label: 'FP16 FLOP/s', num: 1, get: function (r) { return r.fp16_flops ? r.fp16_flops.toExponential(2) : ''; } },
      { label: 'List price', num: 1, get: function (r) { return r.price_usd ? usd(r.price_usd) : ''; } }, { label: 'FLOP/s per $', num: 1, get: function (r) { return r.flops_per_usd ? compact(r.flops_per_usd) : ''; } }],
  ...
  function renderTests(parent) {
    var d = D.epoch_benchmarks_internal, c = card(parent, 'm-tests', 'Hard tests, running out of room',
      'Share of questions right on three benchmarks Epoch runs itself. When the best score nears the ceiling, the test stops telling the leaders apart, and a new test takes over.');
    var pos = S.positions.epoch_benchmarks_internal;
    var n = h('div', 'now', null, c);
    pos.items.forEach(function (it) { h('span', 'who', it.name + ': best ' + pct(it.value) + ' (' + it.who + ', ' + fmtDate(it.date) + ')', n); });
    ...
    d.benchmarks.forEach(function (b) {
      ...
      mount(host, 'xyChart', function () {
        pts.forEach(function (p) { p.color = kindColor(p.kind); });
        var layers = [{ type: 'rule', y: 1, label: 'all correct' }];
        if (b.random_baseline) layers.push({ type: 'rule', y: b.random_baseline, label: 'guessing' });
        if (b.score_ceiling && b.score_ceiling < 1) layers.push({ type: 'rule', y: b.score_ceiling, label: 'ceiling' });
        ...
        return {
          ...
          x: { min: Math.min.apply(null, xs) - 0.1, max: xmax },
          y: { min: 0, max: 1.04, fmt: function (v) { return Math.round(v * 100) + '%'; } },
  ...
  function renderMetr(parent) {
    ...
    p.appendChild(document.createTextNode('The chart lives on METR\'s own page, because no open licence covers its per-model numbers: '));
    link(d.page, 'metr.org/time-horizons', p, true);
    p.appendChild(document.createTextNode('. Quoted here, with attribution, are only METR\'s own fitted doubling times:'));
    ...
    d.quoted.forEach(function (q) {
      h('li', null, 'Version ' + q.version.slice(1) + ', ' + q.window + ': doubles every ' + (q.doubling_days / 30.44).toFixed(1) + ' months' +
        (q.ci_low_days ? ' (range ' + (q.ci_low_days / 30.44).toFixed(1) + ' to ' + (q.ci_high_days / 30.44).toFixed(1) + ')' : '') + (q.note ? '. ' + q.note + '.' : '.'), ul);
    });
    h('p', 'fitline muted', 'The two versions of METR\'s task suite give different answers; both are listed, neither is averaged away. Newest model measured: ' +
  ...
  function renderRevenue(parent) {
    var d = D.epoch_ai_companies, c = card(parent, 'm-revenue', 'Revenue',
      'Reported yearly revenue run-rates of companies whose main business is AI models. Lines join each company\'s reports; the bands are the fitted trends for the two with the most reports.');
    ...
    mount(host, 'xyChart', function () {
      ...
      return {
        ...
        marginRight: function (W) { return 84; },
    ...
    legend(c, [{ label: 'OpenAI', color: css('--s3') }, { label: 'Anthropic', color: css('--s7') }, { label: 'other companies (hover or table)', color: css('--neutral-soft') },
      { label: 'fitted trend, 90% band', shape: 'band' }]);
  ...
  function renderCost(parent) {
    ...
    nowLine(c, usd(pos.value), 'costliest estimate: ' + pos.who + ' (' + pos.org + ', ' + fmtDate(pos.date) + ', compute "' + pos.confidence + '")');
    ...
    var xs = pts.map(function (p) { return p.x; });
    mount(host, 'xyChart', scatterSpec({
      aria: 'Training run cost of notable models, log scale, with the frontier trend',
      points: function () { pts.forEach(function (p) { p.color = kindColor(p.kind); }); return pts; },
    ...
    kindLegend(c, [{ label: 'hollow: "Speculative" compute estimate', color: css('--ink-3'), shape: 'hollow' }, { label: 'fitted trend (frontier), 90% band', shape: 'band' }]);
    h('p', 'fitline', fitSentence(trend('cost_frontier'), 'Frontier models since 2016'), c);
    coverage(c, d);
  ...
  function renderPlayerPanel() {
    ...
    h('p', 'kind', p.kind + ' · ' + p.country + '. Marked in every chart below; everyone else is faded.', box);
    var ul = h('ul', null, null, box);
    function line(meter, anchor, text, absent) {
      var li = h('li', null, null, ul), m = h('span', 'm', null, li);
      link('#' + anchor, meter, m);
      h('span', absent ? 'absent' : '', text, li);
    }
    var mine = function (rows, key) { return rows.filter(function (r) { return playerOf(r[key || 'org']) === id; }); };
    var tc = mine(objs(D.epoch_training_compute)), withF = tc.filter(function (r) { return r.flop; });
    var cov = D.epoch_training_compute.coverage.numbers, recentN = tc.filter(function (r) { return r.date >= cov.recent_since; });
    if (withF.length) {
      var b = best(withF, 'flop');
      line('Training compute', 'm-compute', 'largest estimate ' + sci(b.flop) + ' FLOP (' + b.model + ', ' + fmtDate(b.date) + ', "' + b.confidence + '"). Estimates for ' +
        recentN.filter(function (r) { return r.flop; }).length + ' of their ' + recentN.length + ' notable models since ' + fmtDate(cov.recent_since) + '.');
    } else line('Training compute', 'm-compute', tc.length ? 'no public estimate for any of their ' + tc.length + ' notable models' : 'no notable models listed', true);
    ...
    if (metr) {
      var vs = metr.coverage.numbers.versions, found = vs.map(function (v) {
        var n = Object.keys(v.orgs).filter(function (o) { return playerOf(o) === id; }).reduce(function (s, o) { return s + v.orgs[o]; }, 0);
        return n ? n + ' in ' + v.version : null;
      }).filter(Boolean);
    ...
    if (sales.length) { var ls = sales.sort(function (a, b) { return b.quarter_end.localeCompare(a.quarter_end); })[0]; line('Chips shipped', 'm-shipped', compact(ls.q_median) + ' H100e shipped in the quarter to ' + fmtDate(ls.quarter_end) + '; ' + compact(ls.cum_median) + ' since ' + fmtDate(ls.start)); }
    ...
    if (mineOwn.length) { var so = mineOwn.reduce(function (s, r) { return s + r.median; }, 0); line('Compute held', 'm-owners', compact(so) + ' H100e, about ' + pct(so / total) + ' of what is counted (to ' + fmtDate(q) + ')'); }
    else line('Compute held', 'm-owners', 'not listed as an owner (may rent, or be counted inside ' +
      (p.country === 'China' ? '"China" or "Other"' : '"Other"') + ')', true);
    var hw = objs(D.epoch_ml_hardware).filter(function (r) { return playerOf(r.maker) === id; });
    if (hw.length) { var lh = hw[hw.length - 1]; line('Chips designed', 'm-hardware', hw.length + ' chips listed; newest ' + lh.name + ' (' + fmtDate(lh.date) + ')' + (lh.flops_per_usd ? '' : ', no public price')); }
    var rev = objs(D.epoch_ai_companies).filter(function (r) { return playerOf(r.company) === id; });
    if (rev.length) { var lr = rev.sort(function (a, b) { return b.date.localeCompare(a.date); })[0]; line('Revenue', 'm-revenue', usd(lr.annualized_usd) + ' a year (' + fmtDate(lr.date) + ', ' + (lr.source_type || 'reported') + ')'); }
    else line('Revenue', 'm-revenue', 'no reported AI revenue figure', true);
    var links = h('p', 'links', null, box);
    if (p.lab_page) { link(p.lab_page, 'Our page on ' + p.name, links); links.appendChild(document.createTextNode(' · ')); }
    (p.own_words || []).forEach(function (w) { link(w.url, w.title, links, true); links.appendChild(document.createTextNode(' · ')); });
    link(p.home, 'Home page', links, true);
    h('p', 'kind', 'Links in their own words, checked ' + fmtDate(P.checked, true) + '. What they say is their view; the meters above are other people\'s measurements.', box);
  }
  function writeHash() {
    var parts = [];
  ...
  function renderSources() {
    ...
    IDS.forEach(function (id) {
      ...
      h('td', null, d.redistribution === 'link-only' ? 'link only' : 'copied, with credit', tr);
      h('td', null, fmtDate(d.fetched_at, true), tr);
      var tf = h('td', null, null, tr); link('data/' + id + '.json', 'data', tf); tf.appendChild(document.createTextNode(' · ')); link('scripts/fetch_' + id + '.py', 'fetcher', tf);
    });
  }
