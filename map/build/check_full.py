#!/usr/bin/env python3
"""Sanity checks for build_full.py's output (map/data/), adapted from check.py for the whole-corpus
build. Same mechanical rules (snippet caps, no caption markup/speaker-label leakage, idea links not
mid-word, size budgets, no absolute paths/account name in map/), MINUS check.py's quote-review gate:
this round's snippets were not read by a person (see build_full.py's docstring and index.html), so
there is no build/quote-review.json to check them against.

    python map/build/check_full.py
"""
...
dims = info.get('search_vectors', {}).get('dims', 384)

for k_ in ('ep', 't0', 't1', 'x', 'y', 'c', 'ideas'):
    if len(P[k_]) != N:
        fail(f'passages.{k_} has {len(P[k_])} rows, expected {N}')
for i in range(N):
    e = P['ep'][i]
...
for k_, s in snips.items():
    ...
    n = words(s['t'])
    if n > cap:
        fail(f'snippet {k_} ({ep["show"]}) has {n} words, cap {cap}')
    if re.search(r'>>|\[[A-Za-z ]+\]', s['t']):
        fail(f'snippet {k_} still carries caption markup')
    if re.search(r'(?:^|[.?!…] )[A-Z][a-z]+ [A-Z][a-z]+: ', s['t'].lstrip('…')):
        fail(f'snippet {k_} still carries a speaker label: {s["t"][:60]!r}')
    for a, b, _ in s['m']:
        if (a > 0 and s['t'][a - 1].isalnum()) or (b < len(s['t']) and s['t'][b].isalnum()):
            fail(f'snippet {k_} has an idea link that stops mid-word')
    used[P['ep'][i]] += n
for e in E:
...
for root, dirs, files in os.walk(MAP):
    ...
    for f in files:
        ...
        if m and rel not in ('build/check.py', 'build/check_full.py'):
...
budget = {
    ...
    'data/vectors.i8.bin': (os.path.getsize, dims * N),
    'data/scale.f32.bin': (os.path.getsize, 4 * N),
    'data/pca.f32.bin': (os.path.getsize, 4 * 384 * (dims + 1) + 1000),
    'data/build-info.json': (os.path.getsize, 6_000),
...
for rel, (fn, lim) in budget.items():
    ...
    if rel.startswith('data/vectors') or rel.startswith('data/scale') or rel.startswith('data/pca') or rel.startswith('model/'):
        total_lazy += os.path.getsize(p)
print(f'  lazy-loaded total (model + search vectors): {total_lazy:,} B ({total_lazy/1e6:.1f} MB)')
...
print(f'  index.json (eager load) gz size vs ~1.5 MB target: {gz(os.path.join(DATA, "index.json")):,} B')
if fails:
    print('CHECK FAILED')
    for f in fails:
        print('  -', f)
    sys.exit(1)
print('check_full.py: all mechanical rules pass (no manual quote review this round)')
