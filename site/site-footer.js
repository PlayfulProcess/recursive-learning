/* Recursive Eco-Improvement — one shared footer: under construction · her spiral · newsletter.
   ...
   the `base` attribute the copied viewers still pass is read by nothing. The newsletter writes to recursive.eco's
...
(function(){
  ...
  .rtf button:hover{background:#126445}.rtf button:disabled{opacity:.6;cursor:default}
      ...
      f.addEventListener('submit', async function(e){
        ...
        try{
          var r=await fetch(SUPA+'/rest/v1/newsletter_subscribers',{method:'POST',
            ...
            body:JSON.stringify({email:email,subscribed_from:'recursive-learning',subscribed:true})});
          if(r.ok){msg.style.color='#3f7a5c';msg.textContent='Welcome. You are on the list.';f.querySelector('#rtf-email').value='';}
          else if(r.status===409){msg.style.color='#3f7a5c';msg.textContent='You are already on the list.';}
